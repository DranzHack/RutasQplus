...
loadAgain = (t) => {
                    var form = new FormData(DatosAlv);
                    Parametro = '../../XML/XmlDinamico.php';
                    t!=document.getElementById("corrida")?loadCorridas(form):0;
                    Array.from(form).length == 3?initMap(form):0;
                  }

function initMap(form=false) {
            var map = new google.maps.Map(document.getElementById('map'), {
              center: new google.maps.LatLng(19.0583202, -98.2122048),
              zoom: 12
            });
            
              if(Array.from(form).length == 3){
                Equipo1(form, Parametro, function(data) {
      ...
      for(var base6 in bases6)
      {
        // Add the circle for this city to the map.
          ...
          function Equipo1(form, url, callback) {
            var request = window.ActiveXObject ?
                new ActiveXObject('Microsoft.XMLHTTP') :
                new XMLHttpRequest;
          ...
          function loadCorridas(form) {
            ...
            request.onreadystatechange = function() {
              if (request.readyState == 4 && request.status == 200) {
                corrida.innerHTML = request.responseText;
              }
            };
    
            request.open('POST', '../../controlador/ComboCorridaUnidad.php', true);
