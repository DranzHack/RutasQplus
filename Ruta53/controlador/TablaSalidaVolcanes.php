<?php 
    ...
    $LaWea = new Scouts();
    $registros1 = $LaWea->MostrarSalidaVolvanes();         
    ...
    else
    {
        ...
        while($row=sqlsrv_fetch_array($registros1))
        {
            ...
            <td>'.$row['Vl_Vuelta'].'</td>
            <td>
              <button id='.$row['En_IdEnrolado'].' data-id='.$row['En_IdEnrolado'].' data-toggle="modal" data-target="#editModalVolcanes"  title='.$row['En_IdEnrolado'].' type="button" class="editVolcanes btn btn-info">Actualizar</button>
